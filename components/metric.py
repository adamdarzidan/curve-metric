...
class Metric:
    ...
    def score(self, text: str, row: Dict[str, Optional[float]] | None = None, debug = False, print_shap: bool = False) -> tuple[float, dict, np.ndarray]:
        # Extract raw document profile
        df: DocumentProfile = self.fp.extract(text)

        # Convert DocumentProfile to feature vector (same as training)
        features = []
        profile_dict = {}
        ...
        if print_shap:
            self.__print_shap_for_prediction(shap_values[0])

        return predicted[0], profile_dict, shap_values[0]
    ...
    def __get_feature_names(self) -> list[str]:
        ...
        """
        feature_names = []
        for f in fields(DocumentProfile):
            feature_names.extend([f"{f.name}_avg", f"{f.name}_sd", f"{f.name}_min", f"{f.name}_max"])
        ...
        feature_names.extend(tabular_names)
    ...
    def __print_shap_for_prediction(self, shap_values: np.ndarray):
        feature_names = self.__get_feature_names()
